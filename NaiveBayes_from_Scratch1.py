...
OUTPUT_BUCKET = 'gs://p1bucket/output.txt'
...
def rdd_fix(rdd):
    rdd = rdd.flatMap(lambda x: x.split()).filter(lambda x: clean(x)).map(lambda x: (x, 1)).reduceByKey(lambda a,b: a+b)
...
def generate_count_rdds(mapper, trainset=True):
    ...
    for k, v in mapper.items():
        if (v in files_rdds.keys()) and (trainset):
            ...
            files_rdds[v] = files_rdds[v].leftOuterJoin(files_rdds[v]).map(lambda x: (x[0], list(filter(None, x[1])))).map(lambda x: (x[0], sum(x[1]))).cache()
    ...
    for k, v in files_rdds.items():
        if trainset:
            word_perClass[k] = files_rdds[k].values().sum()
...
# init output file
output_path = 'gs://p1bucket/output.txt'
# --- step 1 --- #
mapper = map_datasets2labels(sc, 'X_train.txt', 'y_train.txt')  
# --- step 2 --- #
files_rdds, class_count, word_perClass = generate_count_rdds(mapper)
############### NOTE ###################
# if you want bigrams run this instead of above 
# files_rdds, class_count, word_perClass = generate_count_rdds_bigram(mapper)
...
# load test data
# --- step 1 --- #
mapper_test = map_datasets2labels(sc, 'X_test.txt', '0')  
# --- step 2 --- #
files_rdds_test, class_count_test, _VOID = generate_count_rdds(mapper_test, trainset=False)

############### NOTE ###################
# if you want bigrams run this instead of above 
# files_rdds_test, class_count_test, _VOID = generate_count_rdds_bigram(mapper_test, trainset=True)
...
for rdd_key in files_rdds_test.keys(): # test rdd's formatted key:'path', value: words E.G. ("00", "BG" .... "01")
    ...
    for k, v in class_count.items():
        ...
        try:
            scores[k] = files_rdds_test[rdd_key].map(lambda x: score_calc_fast(x[0], x[1])).reduce(lambda x, y: x +y)
    ...
    print(best_class)
    if predictions == []:
        predictions = [best_class]
    else:
        predictions.append(best_class)
   
preds = sc.parallelize(predictions).flatMap(lambda x: x)
preds.saveAsTextFile(OUTPUT_BUCKET)
